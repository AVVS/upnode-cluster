 ...
 * @param {Function} next
 */
Node.prototype.acquireResource = function (resourceId, resourceType, args, next) {
    var self = this;
        ...
        .catch(Errors.NotFound, function localResourceNotFound() {
            ...
            return Promise.fromNode(function acquireRemoteResource(callback) {
                ...
                self._queue.push(resourceKey, callback, function lockAcquisition(err, workCompleted) {
                    ...
                    // perform operation
                    self.askPeers(resourceId, resourceType, args)
                        .catch(Errors.NotFound, function noPeerHoldsResource() {
                            return self.createResource(resourceId, resourceType, args);
                        })
        ...
        .then(function (remote) {
            Promise.fromNode(function (callback) {
                ...
                function responded(err, response) {
                    remote.removeListener('down', onRemoteDown);
                    callback(err, response);
                }

                remote.once('down', onRemoteDown);
...
Node.prototype.invoke = function (resourceId, resourceType, args, next) {
    ...
    return Promise.fromNode(function (callback) {
        resourceHolder.invoke(resourceId, args, callback);
    }).nodeify(next || _.noop);
};
...
Node.prototype.createResource = function (resourceId, resourceType, args, next) {
    ...
    if (!resource) {
        resource = this._resources[resourceType] = this._resource(resourceType);
    }

    return Promise.fromNode(function (done) {
        return resource.create(resourceId, args, done);
    }).nodeify(next || _.noop);
};
...
Node.prototype.hasResource = function (resourceId, resourceType, args, next) {
    ...
    if (resourceHolder) {
        return Promise.fromNode(function (done) {
            resourceHolder.get(resourceId, args, done);
        }).nodeify(next || _.noop);
    }
...
Node.prototype._discoveredServer = function (channel, message) {
    ...
    peers[connectionString] = new Peer(_.extend(parsed, this._options.peer));
};
